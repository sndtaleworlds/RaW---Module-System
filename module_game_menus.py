...
game_menus = [
  ...
  ##diplomacy begin
    "Welcome to the Rome at War Singleplayer campaign, this version of the campaign is still considered early alpha implying that bugs and unfinished features are to be expected.",
  ##diplomacy end
  "none",
    [],
    [
  ...
  ("start_phase_2",mnf_disable_all_keys,
    "Adventurer, please select your starting position...",
    "none",
    [],
    [
      ("start_1",[(eq,"$background_type",cb_noble)],"Start near your fief",
       [
          	      (call_script, "script_lord_get_home_center", "trp_player"),
				  (assign, ":home_center", reg0),	
				  (party_relocate_near_party, "p_main_party", ":home_center", 3),		
         (change_screen_return),				 
       ]),	   
	   
################################################################# For others:
      ("town_1",[ (neq,"$background_type",cb_noble)],"Start near Roma [Rome]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_1", 2),
       (change_screen_return),
       ]),
      ("town_2",[ (neq,"$background_type",cb_noble)],"Start near Maloenton [South Italy]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_10", 2),
       (change_screen_return),
       ]),
      ("town_3",[ (neq,"$background_type",cb_noble)],"Start near Taras [Greek colonies of South Italy]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_17", 2),
       (change_screen_return),
       ]),
      ("town_4",[ (neq,"$background_type",cb_noble)],"Start near Syrakousai [Sicily]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_23", 2),
       (change_screen_return),
       ]),
      ("town_5",[ (neq,"$background_type",cb_noble)],"Start near Tarchuna [Etruria]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_30", 2),
       (change_screen_return),
       ]),
      ("town_6",[ (neq,"$background_type",cb_noble)],"Start near Mediolanon [Celts]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_45", 2),
       (change_screen_return),
       ]),
      ("town_7",[ (neq,"$background_type",cb_noble)],"Start near Athenai [Greece]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_50", 2),
       (change_screen_return),
       ]),
      ("town_8",[ (neq,"$background_type",cb_noble)],"Start near Pella [Macedonia]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_65", 2),
       (change_screen_return),
       ]),
      ("town_9",[ (neq,"$background_type",cb_noble)],"Start near Ambrakia [Epirus]",
       [
       (party_relocate_near_party, "p_main_party", "p_town_76", 2),
       (change_screen_return),
       ]),
...
##      ]),
      ...
      [
        ...
        (troop_clear_inventory, "$g_player_troop"),
        (troop_add_item, "$g_player_troop","itm_a_linothorax",0),
        (troop_add_item, "$g_player_troop","itm_b_iphicratids",0),
        # (troop_add_item, "$g_player_troop","itm_practice_sword",0),
        (troop_add_item, "$g_player_troop","itm_quarter_staff",0),
        (troop_equip_items, "$g_player_troop"),
        (set_visitor,0,"trp_player"),
  ...
  ("reports",0,
  "Character Renown: {reg5}^Honor Rating: {reg6}^Party Morale: {reg8}^Party Size Limit: {reg7}^Next pay time: {s1}",
   "none",
   [(call_script, "script_game_get_party_companion_limit"),
    ...
    (try_begin),
      ...
      (str_store_string, s2, "@{s1}^^Affiliated to {s5}"),
    (try_end),
    ##diplomacy end
   ],
    [
...
##diplomacy end+
  ...
  (
    ...
    "As a noble you will start in the midst of a faction, granted a fief, troops, wealth, and some skills in order to jump right in the action.^^ As a warrior you will start with some troops, wealth, and skills saving you a bit of the initial grind, you can get a mercenary contract from the factions and join them freely as you see fit.^^ As a commoner you will start at the bottom similar to Warband Native, you will just have the choice of how deep the bottom is.",
    "none",
    [
    (str_clear,s10),
    (str_clear,s11),
    ...
    [
    ("start_noble",[],"Start as a Noble.",[
      (assign,"$background_type",cb_noble),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a noble you start in the service of one of the factions with a fief, troops, wealth and some skills in order to jump right in the action."),
	(jump_to_menu,"mnu_start_character_noble"),
    ]),
    ("start_warrior",[],"Start as a Warrior.",[
      (assign,"$background_type",cb_warrior),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a warrior you start with a small band of troops, some wealth and skills saving you a bit of the initial grind."),
	(jump_to_menu,"mnu_start_character_warrior"),
    ]),
    ("start_commoner",[],"Start as a Filthy Pleb.",[
      (assign,"$background_type",cb_commoner),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a commoner you start at the bottom similar to Warband Native."),
	(jump_to_menu,"mnu_start_character_commoner"),
    ]),
    ("go_back",[],"Go back",
     [(jump_to_menu,"mnu_start_game_1"),
    ]),
 ...
 (
    ...
    "{s10}^^ Select your faction...",
    "none",
    [],
    [
      ("roma",[
          ],"Romani [Romans]",[
      (assign,"$background_answer_2", cb2_rome),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@In 300 B.C. Rome is on the verge of starting its first major expansion now that most of Latium is under Roman control. The Greeks and the Oscan tribes of Campania, fearing the Samnites' thirst for conquest, became protectorates of Rome adding considerable riches and territory to the Republic. In the south the bellicose Samnites are not taking kindly to their neighbors' plans and a third war is on its way to resolve the dispute once and for all. Rome's former masters from the north, the Etruscans, may well ally with the Samnites again and prove a formidable threat even in their twilight years. In the meantime the fear for Celtic raids, the metus Gallicus, is still vivid in the hearts and eyes of Roman people."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]),
      ("koinon_hellenon",[
          ],"Koinon Hellenon [Greek League]",[
      (assign,"$background_answer_2", cb2_greece),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@In 300 B.C. Greece was far from an united entity and more of a cluster of cities with their own sets of laws and customs. For the sake of convenience, they are a single faction in Rome at War for the time being. With the passing of Alexander the Great, his successors have been relentlessly battling over his conquests, and some territories in Greece are being coveted by the Macedonians. The city-states will be the theatre of many wars to come and must stand together in leagues to ensure their survival against foes superior in numbers."),
   (jump_to_menu,"mnu_select_initial_renown"), 
    ]),
      ("keltoi",[
          ],"Keltoi [Celts]",[
      (assign,"$background_answer_2",cb2_gallia),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@In 300 B.C. the celtic culture is approaching its peak and is still looking forward for new conquests in both spoils and territory. Almost a century ago the Celts have pillaged Rome itself, demanded heavy tribute and successfuly returned home with the spoils of war but it still wasn't enough. They now have returned and taken cities from the weakened Etruscans and, by doing so, rekindled the fear of celtic raids in all Italy."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]),
      ("makedonia",[
          ],"Makedonia [Macedon]",[
      (assign,"$background_answer_2",cb2_makedon),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a noble {reg3?girl:boy} of Makedonia,\
 you dream to restore the former glory and prestige which our people achieved under the glorious leadership of Alexander the Great. For this dream to be fulfilled, Makedon needs a strong and a charismatic leader to unite and inspire the masses. The Basileus is growing old, and obviously can not fulfill this role. We could seize Hellas once and for all by giving them the single, decisive strike, which both Philip and Alexander refused to give in the memory of past glories. May it be your fate, Archon."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]),
      ("samnium",[
          ],"Safinim [Samnites]",[
      (assign,"$background_answer_2",cb2_samnium),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a {reg3?girl:boy} of Samnium,\
 you have earned your rank among the nobility and you have taken the oath, and you acknowledge that there is no way back. You will defend your homeland from every man who dares to threaten it, especially those despiseable Romans in the north and you will show no fear or hesitation while doing it. We have fought them twice already, and they are trying to get the upper hand by copying our ways of fighting. Let us show them that their desperate measures are in vain, Meddix."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]),   
      ("megale_hellas",[
          ],"Megale Hellas [Greek colonies of southern Italy]",[
      (assign,"$background_answer_2",cb2_megale_hellas),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a {reg3?girl:boy} of Taras,\
 you have earned your rank among the nobility but the restless Oscan tribes threaten your existance and grow bolder with time, it must come to an end. You know that they can be defeated as you did many times in the past but this time you must not spare them Cleonymus, a heir to Sparta has come to take the lead of the League against the Italic tribes, may he be a worthy addition to our cause but keep a watchful eye if he becomes too ambitious for our own good. Now go on campaign Lochagos and may the gods watch over you."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]),   
      ("basileion_syrakosion",[
          ],"Syrakousai [Kingdom of Sicily]",[
      (assign,"$background_answer_2",cb2_basileion_syrakosion),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a {reg3?girl:boy} of Syracuse,\
 you live in one of the mightiest cities of the Greek World, trade flourishes and so does your enemies, it wouldn't be wise to underestimate them, the Carthaginians are settling in our lands and the Tarentines of Megale Hellas would be very keen on seizing our trade. Strike first and strike hard."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]),  
      ("epeiros",[
          ],"Epeiros [Epirus]",[
      (assign,"$background_answer_2",cb2_epeiros), 
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a {reg3?girl:boy} of Epirus,\
 your lands are slowly growing in power, your warriors serve as mercenaries in the Macedonian armies adquiring both wealth and experience which once they return home it provides you with well trained army, with the Greek world at war it may be time to claim your place among the great empires but do not let ambition conquer you."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]), 
      ("etruria",[
          ],"Rasenna [Etruscans]",[
      (assign,"$background_answer_2",cb2_etruria),
      (assign, reg3, "$character_gender"),
      (str_store_string,s11,"@As a {reg3?girl:boy} of Rasenna,\
 once a prideful people and major power in Italy is now in its twilight days. The celtic migrations led to countless invasions and wars and now the Romans threaten to take your lands. It will take the greatest of men and the strongest of will to reverse the balance of power."),
   (jump_to_menu,"mnu_select_initial_renown"),
    ]),   	
      ("go_back",[],"Go back.",
     [(jump_to_menu,"mnu_start_character_1"),
  ...
  (
    "select_initial_renown",0,
    "^^ Select your renown...",
    "none",
    [],
    [    
      ("Legend",[(eq,"$background_type",cb_noble)],
	  "Legend [500]",[
        (troop_set_slot, "trp_player", slot_troop_renown, 500),  
  (jump_to_menu,"mnu_select_initial_wealth"),
    ]),   	
      ("Hero",[(eq,"$background_type",cb_noble)],
	  "Hero [400]",[
        (troop_set_slot, "trp_player", slot_troop_renown, 400),  
  (jump_to_menu,"mnu_select_initial_wealth"),
    ]),   	
      ("Popular",[(eq,"$background_type",cb_noble)],
	  "Popular [200]",[
  ...
  (
    "select_initial_wealth",0,
    "^^ Select your wealth...",
    "none",
    [],
    [  
      ("Kroisos",[(eq,"$background_type",cb_noble)],
	  "Rich like Kroisos [99 999]",[
        (troop_add_gold, "trp_player", 99999),		  
  (jump_to_menu,"mnu_noble_choose_fief"),
    ]),   
      ("Rich",[(eq,"$background_type",cb_noble)],
	  "Rich Nobleman [25 000]",[
        (troop_add_gold, "trp_player", 25000),		  
  (jump_to_menu,"mnu_noble_choose_fief"),
    ]),     
      ("Wealthy",[(eq,"$background_type",cb_noble)],
	  "Wealthy Nobleman [10 000]",[
        (troop_add_gold, "trp_player", 10000),		  
  (jump_to_menu,"mnu_noble_choose_fief"),
    ]), 
      ("Wealthy_poor",[(eq,"$background_type",cb_noble)],
	  "Poor Nobleman [5 000]",[
        (troop_add_gold, "trp_player", 5000),		  
  (jump_to_menu,"mnu_noble_choose_fief"),
    ]), 	
      ("high",[(neq,"$background_type",cb_noble)
          ],"Rich Citizen [5 000]",[
        (troop_add_gold, "trp_player", 5000),		  
  (jump_to_menu,"mnu_choose_skill"),
    ]), 		
      ("Average_high",[(neq,"$background_type",cb_noble)
          ],"Upper-Class Citizen [2 000]",[
        (troop_add_gold, "trp_player", 2000),		  
  (jump_to_menu,"mnu_choose_skill"),
    ]), 	
      ("Average",[(neq,"$background_type",cb_noble)
          ],"Middle-Class Citizen [500]",[
        (troop_add_gold, "trp_player", 500),		  
  (jump_to_menu,"mnu_choose_skill"),
    ]), 	 
      ("Low",[(neq,"$background_type",cb_noble)
          ],"Lower-Class Citizen [200]",[
        (troop_add_gold, "trp_player", 200),		  
  (jump_to_menu,"mnu_choose_skill"),
    ]), 	
      ("Poor",[(neq,"$background_type",cb_noble)
          ],"Found some coins on the ground [5]",[
        (troop_add_gold, "trp_player", 5),		  
  (jump_to_menu,"mnu_choose_skill"),
    ]), 		
      ("go_back",[],"Go back.",
     [(jump_to_menu,"mnu_start_character_1"),
...
# Rome	
      ...
      # (str_store_string,s11,"@You rule over Roma"),
	# (jump_to_menu,"mnu_choose_skill"),
    # ]),  	
      ("tibur",[
      (eq,"$background_answer_2", cb2_rome),
      ...
      (str_store_string,s11,"@You rule over Tibur"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Tusculum",[
      (eq,"$background_answer_2", cb2_rome),
      ...
      (str_store_string,s11,"@You rule over Tusculum"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
      ("antium",[
      (eq,"$background_answer_2", cb2_rome),
      ...
      (str_store_string,s11,"@You rule over Antium"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Capua",[
      (eq,"$background_answer_2", cb2_rome),
          ],"Capua",[
      (call_script, "script_give_center_to_lord", "p_town_5", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Capua"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Nola",[
      (eq,"$background_answer_2", cb2_rome),
      ...
      (str_store_string,s11,"@You rule over Nola"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Neapolis",[
      (eq,"$background_answer_2", cb2_rome),
      ...
      (str_store_string,s11,"@You rule over Neapolis"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Cumae",[
      (eq,"$background_answer_2", cb2_rome),
          ],"Cumae",[
      (call_script, "script_give_center_to_lord", "p_town_8", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Cumae"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Pompeii",[
      (eq,"$background_answer_2", cb2_rome),
          ],"Pompeii",[
      (call_script, "script_give_center_to_lord", "p_town_9", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Pompeii"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
# Samnites	
      ("Maloenton",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_10", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Maloenton"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Ladinod",[
      (eq,"$background_answer_2", cb2_samnium),
          ],"Ladinod",[
      (call_script, "script_give_center_to_lord", "p_town_11", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Ladinod"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Buvaianud",[
      (eq,"$background_answer_2", cb2_samnium),
      ...
      (str_store_string,s11,"@You rule over Buvaianud"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Venusia",[
      (eq,"$background_answer_2", cb2_samnium),
          ],"Venusia",[
      (call_script, "script_give_center_to_lord", "p_town_13", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Venusia"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Histonion",[
      (eq,"$background_answer_2", cb2_samnium),
          ],"Histonion",[
      (call_script, "script_give_center_to_lord", "p_town_14", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Histonion"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Aisernio",[
      (eq,"$background_answer_2", cb2_samnium),
          ],"Aisernio",[
      (call_script, "script_give_center_to_lord", "p_town_15", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Aisernio"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Tullisiom",[
      (eq,"$background_answer_2", cb2_samnium),
      ...
      (str_store_string,s11,"@You rule over Tullisiom"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
# Megale Hellas	
      ("Taras",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_17", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Taras"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Thourioi",[
      (eq,"$background_answer_2", cb2_megale_hellas),
          ],"Thourioi",[
      (call_script, "script_give_center_to_lord", "p_town_18", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Thourioi"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Kroton",[
      (eq,"$background_answer_2", cb2_megale_hellas),
      ...
      (str_store_string,s11,"@You rule over Kroton"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Rhegion",[
      (eq,"$background_answer_2", cb2_megale_hellas),
          ],"Rhegion",[
      (call_script, "script_give_center_to_lord", "p_town_20", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Rhegion"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Lokroi_Epizephyrioi",[
      (eq,"$background_answer_2", cb2_megale_hellas),
          ],"Lokroi Epizephyrioi",[
      (call_script, "script_give_center_to_lord", "p_town_21", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Lokroi Epizephyrioi"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Elaia",[
      (eq,"$background_answer_2", cb2_megale_hellas),
      ...
      (str_store_string,s11,"@You rule over Elaia"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
# Sicily	
      ("Syrakousai",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_23", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Syrakousai"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
      ("Messana",[
      (eq,"$background_answer_2", cb2_basileion_syrakosion),
          ],"Messana",[
      (call_script, "script_give_center_to_lord", "p_town_24", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Messana"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
      ("Tauromenion",[
      (eq,"$background_answer_2", cb2_basileion_syrakosion),
          ],"Tauromenion",[
      (call_script, "script_give_center_to_lord", "p_town_25", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Tauromenion"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
      ("Akragas",[
      (eq,"$background_answer_2", cb2_basileion_syrakosion),
          ],"Akragas",[
      (call_script, "script_give_center_to_lord", "p_town_26", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Akragas"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
      ("Herakleia_Minoa",[
      (eq,"$background_answer_2", cb2_basileion_syrakosion),
          ],"Herakleia Minoa",[
      (call_script, "script_give_center_to_lord", "p_town_27", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Herakleia Minoa"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
      ("Morgantina",[
      (eq,"$background_answer_2", cb2_basileion_syrakosion),
          ],"Morgantina",[
      (call_script, "script_give_center_to_lord", "p_town_28", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Morgantina"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
      ("Gelas",[
      (eq,"$background_answer_2", cb2_basileion_syrakosion),
          ],"Gelas",[
      (call_script, "script_give_center_to_lord", "p_town_29", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Gelas"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
# Etruscans	
      ("Tarchuna",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_30", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Tarchuna"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Fufluna",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Fufluna",[
      (call_script, "script_give_center_to_lord", "p_town_31", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Fufluna"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Aritim",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Aritim",[
      (call_script, "script_give_center_to_lord", "p_town_32", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Aritim"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Alalia",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Alalia",[
      (call_script, "script_give_center_to_lord", "p_town_33", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Alalia"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Velzna",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Velzna",[
      (call_script, "script_give_center_to_lord", "p_town_34", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Velzna"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Velathri",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Velathri",[
      (call_script, "script_give_center_to_lord", "p_town_35", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Velathri"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Phersna",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Phersna",[
      (call_script, "script_give_center_to_lord", "p_town_36", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Phersna"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Pisna",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Pisna",[
      (call_script, "script_give_center_to_lord", "p_town_37", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Pisna"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Halethi",[
      (eq,"$background_answer_2", cb2_etruria),
          ],"Halethi",[
      (call_script, "script_give_center_to_lord", "p_town_38", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Halethi"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
# Celts	
      ("Sena",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_39", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Sena"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Pisaurum",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Pisaurum",[
      (call_script, "script_give_center_to_lord", "p_town_40", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Pisaurum"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Ravenna",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Ravenna",[
      (call_script, "script_give_center_to_lord", "p_town_41", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Ravenna"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Genua",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Genua",[
      (call_script, "script_give_center_to_lord", "p_town_42", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Genua"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Dertona",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Dertona",[
      (call_script, "script_give_center_to_lord", "p_town_43", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Dertona"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Taurasia",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Taurasia",[
      (call_script, "script_give_center_to_lord", "p_town_44", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Taurasia"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Mediolanon",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Mediolanon",[
      (call_script, "script_give_center_to_lord", "p_town_45", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Mediolanon"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Bona",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Bona",[
      (call_script, "script_give_center_to_lord", "p_town_46", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Bona"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Brixia",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Brixia",[
      (call_script, "script_give_center_to_lord", "p_town_47", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Brixia"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Cremona",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Cremona",[
      (call_script, "script_give_center_to_lord", "p_town_48", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Cremona"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Verona",[
      (eq,"$background_answer_2", cb2_gallia),
          ],"Verona",[
      (call_script, "script_give_center_to_lord", "p_town_49", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Verona"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
# Greeks	
      ("Athenai",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_50", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Athenai"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Thebai",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Thebai",[
      (call_script, "script_give_center_to_lord", "p_town_51", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Thebai"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Argos",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Argos",[
      (call_script, "script_give_center_to_lord", "p_town_52", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Argos"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Lakedaimon",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Lakedaimon",[
      (call_script, "script_give_center_to_lord", "p_town_53", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Lakedaimon"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Gytheion",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Gytheion",[
      (call_script, "script_give_center_to_lord", "p_town_54", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Gytheion"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Messene",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Messene",[
      (call_script, "script_give_center_to_lord", "p_town_55", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Messene"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Hierapytna",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Hierapytna",[
      (call_script, "script_give_center_to_lord", "p_town_56", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Hierapytna"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Knosos",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Knosos",[
      (call_script, "script_give_center_to_lord", "p_town_57", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Knosos"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Rhodos",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Rhodos",[
      (call_script, "script_give_center_to_lord", "p_town_58", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Rhodos"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Delphoi",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Delphoi",[
      (call_script, "script_give_center_to_lord", "p_town_59", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Delphoi"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Korinthos",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Korinthos",[
      (call_script, "script_give_center_to_lord", "p_town_60", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Korinthos"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Megara",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Megara",[
      (call_script, "script_give_center_to_lord", "p_town_61", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Megara"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Elis",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Elis",[
      (call_script, "script_give_center_to_lord", "p_town_62", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Elis"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Megalopolis",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Megalopolis",[
      (call_script, "script_give_center_to_lord", "p_town_63", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Megalopolis"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
      ("Tegea",[
      (eq,"$background_answer_2", cb2_greece),
          ],"Tegea",[
      (call_script, "script_give_center_to_lord", "p_town_64", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Tegea"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  			
# Macedonians	
      ("Pella",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_65", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Pella"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Thessalonike",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Thessalonike",[
      (call_script, "script_give_center_to_lord", "p_town_66", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Thessalonike"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Edessa",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Edessa",[
      (call_script, "script_give_center_to_lord", "p_town_67", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Edessa"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Dion",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Dion",[
      (call_script, "script_give_center_to_lord", "p_town_68", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Dion"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Amphipolis",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Amphipolis",[
      (call_script, "script_give_center_to_lord", "p_town_69", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Amphipolis"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Philippoi",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Philippoi",[
      (call_script, "script_give_center_to_lord", "p_town_70", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Philippoi"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Herakleia_Lynkestis",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Herakleia Lynkestis",[
      (call_script, "script_give_center_to_lord", "p_town_71", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Herakleia Lynkestis"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Chalkis",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Chalkis",[
      (call_script, "script_give_center_to_lord", "p_town_72", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Chalkis"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Larissa_Kremaste",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Larissa Kremaste",[
      (call_script, "script_give_center_to_lord", "p_town_73", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Larissa Kremaste"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Pharsalos",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Pharsalos",[
      (call_script, "script_give_center_to_lord", "p_town_74", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Pharsalos"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
      ("Larissa",[
      (eq,"$background_answer_2", cb2_makedon),
          ],"Larissa",[
      (call_script, "script_give_center_to_lord", "p_town_75", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Larissa"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  	
# Epirus
      ("Ambrakia",[
      ...
      (call_script, "script_give_center_to_lord", "p_town_76", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Ambrakia"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
        ("Kerkyra",[
      (eq,"$background_answer_2", cb2_epeiros),
          ],"Kerkyra",[
      (call_script, "script_give_center_to_lord", "p_town_77", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Kerkyra"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
        ("Phoinike",[
      (eq,"$background_answer_2", cb2_epeiros),
          ],"Phoinike",[
      (call_script, "script_give_center_to_lord", "p_town_78", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Phoinike"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  		
        ("Dodona",[
      (eq,"$background_answer_2", cb2_epeiros),
          ],"Dodona",[
      (call_script, "script_give_center_to_lord", "p_town_79", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Dodona"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  
        ("Apollonia",[
      (eq,"$background_answer_2", cb2_epeiros),
          ],"Apollonia",[
      (call_script, "script_give_center_to_lord", "p_town_80", "trp_player", 0),						  
      (str_store_string,s11,"@You rule over Apollonia"),
	(jump_to_menu,"mnu_choose_skill"),
    ]),  
  ...
  (
    "start_character_warrior",mnf_disable_all_keys,
    "{s11}^^ In which fashion do you fight?",
    "none",
    [(assign, reg3, "$character_gender"),],
    [
      ("hoplite",[],"Greek Hoplite",[
        (assign,"$background_answer_2",cb2_hoplite),
      (str_store_string,s14,"@{reg3?daughter:man}"),
      (str_store_string,s11,"@You fight as an hoplite of the greek cities, using a thrusting spear, the doru and a round shield, the hoplon."),
	(jump_to_menu,"mnu_select_initial_renown"),
        ]),
      ("arjos",[],"Gallic Arjos.",[
        (assign,"$background_answer_2",cb2_arjos),
      (str_store_string,s14,"@{reg3?daughter:man}"),
      (str_store_string,s13,"@{reg3?woman:man}"),
      (str_store_string,s11,"@You fight in the way of the celtic Arjos, your blows are fast an terrible and your ennemy fears you."),
	(jump_to_menu,"mnu_select_initial_renown"),
        ]),
      ("phalangite",[],"Macedonian Phalangite.",[
        (assign,"$background_answer_2",cb2_phalangite),
      (str_store_string,s14,"@{reg3?daughter:man}"),
      (str_store_string,s13,"@{reg3?woman:man}"),
      (str_store_string,s11,"@You fight with the phalangite gear, your spear is long and terrible but your strenght depends of your men."),
	(jump_to_menu,"mnu_select_initial_renown"),
        ]),
      ("cretan_archer",[],"Cretan Toxotes.",[
        (assign,"$background_answer_2",cb2_cretan_archer),
      (str_store_string,s14,"@{reg3?daughter:man}"),
      (str_store_string,s13,"@{reg3?woman:man}"),
      (str_store_string,s11,"@You fight with the cretan gear, your bow is strong and terrible but your strenght depends of your men."),
	(jump_to_menu,"mnu_select_initial_renown"),
        ]),		
      ("roman_hastatus",[],"Roman Hastatus.",[
        (assign,"$background_answer_2",cb2_roman_hastatus),
      (str_store_string,s14,"@{reg3?daughter:man}"),
      (str_store_string,s13,"@{reg3?woman:man}"),
      (str_store_string,s11,"@You fight with the hastatus gear, your sword is strong and terrible but your strenght depends of your men."),
	(jump_to_menu,"mnu_select_initial_renown"),
        ]),			
      ("go_back",[],"Go back.",
       [(jump_to_menu,"mnu_start_character_1"),
  ...
  (
    "start_character_commoner",mnf_disable_all_keys,
    "{s12}^^You are a man of common birth, used to rough life and hardship... But what is your occupation?",
    "none",
    [],
    [
      ("farmer",[],"Farmer.",[
        (assign,"$background_answer_2", cb2_farmer),
      (str_store_string,s11,"@You spent most of your life working on your farm. But one day the bandits came and slaughtered your familly...\
	  You want justice. What was done to you cannot be undone,\
 and these debts can only be paid in blood..."),
        (jump_to_menu,"mnu_select_initial_renown"),
        ]),
      ("doctor",[],"Doctor.",[
        (assign,"$background_answer_2",cb2_doctor),
      (str_store_string,s11,"@You devote your life to the wellfare of other people, saving children and old men alike from diseases.\
	  You know how to heal a man, but also how to kill one..."),
        (jump_to_menu,"mnu_select_initial_renown"),
        ]),
      ("merchant",[],"Merchant.",[
        (assign,"$background_answer_2",cb2_merchant),
      (str_store_string,s11,"@You are a merchant, your only god is coin, you'll do anything to have more\
	  and you will keep it no matter the costs..."),
        (jump_to_menu,"mnu_select_initial_renown"),
        ]),
      ("go_back",[],"Go back.",
       [(jump_to_menu,"mnu_start_character_1"),
  ...
  (
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   (try_begin),
		(eq, ":random_helmet", 0),
        (troop_add_item, "trp_player","itm_h_apulo_a_lf"),
	   (else_try),
		(eq, ":random_helmet", 1),						
        (troop_add_item, "trp_player","itm_h_rome_boetian_c1"),
	   (else_try),
		(eq, ":random_helmet", 2),								
        (troop_add_item, "trp_player","itm_h_roman_montefortino_a_plus_red"),	
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h_boiotian"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h_rome_boetian_d"),								
	   (try_end),	
	...
	(store_random_in_range, ":random_sword", 0, 5),							
	   (try_begin),
		(eq, ":random_sword", 0),
        (troop_add_item, "trp_player","itm_w_italianxiphos"),
	   (else_try),
		(eq, ":random_sword", 1),						
        (troop_add_item, "trp_player","itm_w_espasa"),
	   (else_try),
		(eq, ":random_sword", 2),						
        (troop_add_item, "trp_player","itm_w_italian_xiphos_cav"),	
	   (else_try),
		(eq, ":random_sword", 3),						
        (troop_add_item, "trp_player","itm_w_italian_xiphos_new"),
	   (else_try),
		(eq, ":random_sword", 4),						
        (troop_add_item, "trp_player","itm_w_gladius_hispaniensis_2"),								
	   (try_end),		

	(store_random_in_range, ":random_shield", 0, 5),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_s_roman_parma_a_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_s_roman_parma_a_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_s_roman_parma_a_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_s_roman_parma_boss_2_d_1"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_s_roman_parma_boss_2_c_2"),								
	   (try_end),	

	(store_random_in_range, ":random_horse", 0, 5),							
	   (try_begin),
		(eq, ":random_horse", 0),
        (troop_add_item, "trp_player","itm_saddle_horse"),
	   (else_try),
		(eq, ":random_horse", 1),						
        (troop_add_item, "trp_player","itm_saddle_horse_white"),
	   (else_try),
		(eq, ":random_horse", 2),						
        (troop_add_item, "trp_player","itm_generic_horse_armored"),	
	   (else_try),
		(eq, ":random_horse", 3),						
        (troop_add_item, "trp_player","itm_generic_horse_armored_white"),
	   (else_try),
		(eq, ":random_horse", 4),						
        (troop_add_item, "trp_player","itm_samnitici_equus"),								
	   (try_end),	 	   
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   (try_begin),
		(eq, ":random_helmet", 0),
        (troop_add_item, "trp_player","itm_h_celtic_5"),
	   (else_try),
		(eq, ":random_helmet", 1),						
        (troop_add_item, "trp_player","itm_h_celtic_12"),
	   (else_try),
		(eq, ":random_helmet", 2),						
        (troop_add_item, "trp_player","itm_h_celtic_13"),	
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h_montefortino_8"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h_montefortino_7"),								
	   (try_end),	

	# (store_random_in_range, ":random_armor", 0, 5),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        # (troop_add_item, "trp_player","itm_a_celtic_tunic_long_cape_folded_white"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        (troop_add_item, "trp_player","itm_a_celtic_tunic_long_cape_folded_tartan_red"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_gallic_hamata"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_e"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_celtic_tunic_long_cape_folded_bordered_red"),								
	   # (try_end),

        (troop_add_item, "trp_player","itm_b_celtic_pants_tartan_green"),	   

	(store_random_in_range, ":random_sword", 0, 5),							
	   (try_begin),
		(eq, ":random_sword", 0),
        (troop_add_item, "trp_player","itm_w_celtic_longsword_noble"),
	   (else_try),
		(eq, ":random_sword", 1),						
        (troop_add_item, "trp_player","itm_w_celtic_longsword_2"),
	   (else_try),
		(eq, ":random_sword", 2),						
        (troop_add_item, "trp_player","itm_w_celtic_longsword_1"),	
	   (else_try),
		(eq, ":random_sword", 3),						
        (troop_add_item, "trp_player","itm_w_celtic_longsword_1"),
	   (else_try),
		(eq, ":random_sword", 4),						
        (troop_add_item, "trp_player","itm_w_celtic_longsword_heavy"),								
	   (try_end),	

	(store_random_in_range, ":random_spear", 0, 5),							
	   (try_begin),
		(eq, ":random_spear", 0),
        (troop_add_item, "trp_player","itm_sp_w_celtic_spear_2"),
	   (else_try),
		(eq, ":random_spear", 1),						
        (troop_add_item, "trp_player","itm_w_celtic_javelin"),
	   (else_try),
		(eq, ":random_spear", 2),						
        (troop_add_item, "trp_player","itm_sp_w_celtic_spear_4"),	
	   (else_try),
		(eq, ":random_spear", 3),						
        (troop_add_item, "trp_player","itm_sp_w_ariomaros_spear"),
	   (else_try),
		(eq, ":random_spear", 4),						
        (troop_add_item, "trp_player","itm_sp_w_gaelaiche_spear"),								
	   (try_end),		   

	(store_random_in_range, ":random_shield", 0, 5),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_gallic_hexagonal_scutum_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_gallic_hexagonal_scutum_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_s_celtic_round_shield_2_v2"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_s_celtic_round_shield_4_v2"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_s_celtic_round_shield_gothic_1_v2"),								
	   (try_end),	

	(store_random_in_range, ":random_horse", 0, 2),							
	   (try_begin),
		(eq, ":random_horse", 0),
        (troop_add_item, "trp_player","itm_celtic_saddle_horse"),
	   (else_try),
		(eq, ":random_horse", 1),						
        (troop_add_item, "trp_player","itm_celtic_saddle_horse_white"),							
	   (try_end),			
	...
	# (store_random_in_range, ":random_helmet", 0, 5),							
	   # (try_begin),
		# (eq, ":random_helmet", 0),
        # (troop_add_item, "trp_player","itm_h_attic_mak"),
	   # (else_try),
		# (eq, ":random_helmet", 1),						
        # (troop_add_item, "trp_player","itm_h_phrygian_mask_a"),
	   # (else_try),
		# (eq, ":random_helmet", 2),						
        # (troop_add_item, "trp_player","itm_h4_thracian"),	
	   # (else_try),
		# (eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h_kausia"),
	   # (else_try),
		# (eq, ":random_helmet", 4),						
        # (troop_add_item, "trp_player","itm_h_phrygian_cheeks"),								
	   # (try_end),	

	# (store_random_in_range, ":random_armor", 0, 5),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        (troop_add_item, "trp_player","itm_a_chiton_purple"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_linothorax_mak_lochagos"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_sidescale_mak"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_parmenion"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_philotas"),								
	   # (try_end),

	(store_random_in_range, ":random_footwear", 0, 5),							
	   (try_begin),
		(eq, ":random_footwear", 0),
        (troop_add_item, "trp_player","itm_b_iphicratids"),
	   (else_try),
		(eq, ":random_footwear", 1),						
        (troop_add_item, "trp_player","itm_greek_cavalry_knemis"),
	   (else_try),
		(eq, ":random_footwear", 2),						
        (troop_add_item, "trp_player","itm_greek_proknemis"),	
	   (else_try),
		(eq, ":random_footwear", 3),						
        (troop_add_item, "trp_player","itm_b_single_greave_greek"),
	   (else_try),
		(eq, ":random_footwear", 4),						
        (troop_add_item, "trp_player","itm_b_double_greaves_greek"),								
	   (try_end),	

	(store_random_in_range, ":random_sword", 0, 5),							
	   (try_begin),
		(eq, ":random_sword", 0),
        (troop_add_item, "trp_player","itm_w_light_kopis_officer"),
	   (else_try),
		(eq, ":random_sword", 1),						
        (troop_add_item, "trp_player","itm_w_kopis_officer"),
	   (else_try),
		(eq, ":random_sword", 2),						
        (troop_add_item, "trp_player","itm_w_heavy_kopis_officer"),	
	   (else_try),
		(eq, ":random_sword", 3),						
        (troop_add_item, "trp_player","itm_w_alexander"),
	   (else_try),
		(eq, ":random_sword", 4),						
        (troop_add_item, "trp_player","itm_w_kopis_bird_light"),								
	   (try_end),		

	(store_random_in_range, ":random_spear", 0, 5),							
	   (try_begin),
		(eq, ":random_spear", 0),
        (troop_add_item, "trp_player","itm_sp_w_greek_doru_3"),
	   (else_try),
		(eq, ":random_spear", 1),						
        (troop_add_item, "trp_player","itm_sp_w_doru_short"),
	   (else_try),
		(eq, ":random_spear", 2),						
        (troop_add_item, "trp_player","itm_sp_w_greek_spear_doru_steel"),	
	   (else_try),
		(eq, ":random_spear", 3),						
        (troop_add_item, "trp_player","itm_sp_w_greek_spear_doru_steel_2"),
	   (else_try),
		(eq, ":random_spear", 4),						
	...
	(store_random_in_range, ":random_shield", 0, 5),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_makedonian_hoplon_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_makedonian_hoplon_flower_1"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_makedonian_hoplon_flower_2"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_makedonian_hoplon_solon"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_makedonian_hoplon_face"),								
	   (try_end),	

	(store_random_in_range, ":random_horse", 0, 5),							
	   (try_begin),
		(eq, ":random_horse", 0),
        (troop_add_item, "trp_player","itm_mak_horse_leopard"),
	   (else_try),
		(eq, ":random_horse", 1),						
        (troop_add_item, "trp_player","itm_saddle_horse_white"),
	   (else_try),
	   ...
	   (else_try),
		(eq, ":random_horse", 4),						
        (troop_add_item, "trp_player","itm_mak_horse_leopard_black"),								
	   (try_end),	 	   
	...
	# (store_random_in_range, ":random_helmet", 0, 5),							
	   # (try_begin),
		# (eq, ":random_helmet", 0),
        # (troop_add_item, "trp_player","itm_greek_pylos_complete"),
	   # (else_try),
		# (eq, ":random_helmet", 1),						
        # (troop_add_item, "trp_player","itm_h_boeotian_v2_crest_3"),
	   # (else_try),
		# (eq, ":random_helmet", 2),						
        # (troop_add_item, "trp_player","itm_h_attic_greek_white"),	
	   # (else_try),
		# (eq, ":random_helmet", 3),						
        # (troop_add_item, "trp_player","itm_h_ptolemy"),
	   # (else_try),
		# (eq, ":random_helmet", 4),						
        # (troop_add_item, "trp_player","itm_h_phrygian_mask_b_crest_colors"),								
	   # (try_end),	
	...
	# (store_random_in_range, ":random_armor", 0, 5),							
	   ...
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_linothorax_v5_red"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_linothorax_v6"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_leonnatus"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_cassander"),								
	   # (try_end),
	...
	(store_random_in_range, ":random_spear", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_spear", 4),						
        (troop_add_item, "trp_player","itm_sp_w_iphicrates_doru"),								
	   (try_end),
	   
	(store_random_in_range, ":random_shield", 0, 5),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_greek_gorgon_hoplon"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_greek_snake_hoplon"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_greek_scorpion_hoplon"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_greek_korinthian_hoplon_pegasus"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_greek_theban_hoplon"),								
	   (try_end),	
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   (try_begin),
		(eq, ":random_helmet", 0),
        (troop_add_item, "trp_player","itm_samnite_attic_5plumes"),
	   (else_try),
		(eq, ":random_helmet", 1),						
        (troop_add_item, "trp_player","itm_samnite_attic_c_crest_red"),
	   (else_try),
		(eq, ":random_helmet", 2),						
        (troop_add_item, "trp_player","itm_h6_samno_attic_crest_black"),	
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h4_samno_attic_f"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h3_samno_attic"),								
	   (try_end),	

	# (store_random_in_range, ":random_armor", 0, 5),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        (troop_add_item, "trp_player","itm_samnite_shirt_9"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_samnite_kardio_8s"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_samnite_kardio_full_2"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_samnite_disk_small_2"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_samnite_musculata_small_4"),								
	   # (try_end),

	# (store_random_in_range, ":random_footwear", 0, 5),							
	   # (try_begin),
		# (eq, ":random_footwear", 0),
        (troop_add_item, "trp_player","itm_b_samnite_sandals"),
	   # (else_try),
		# (eq, ":random_footwear", 1),						
        # (troop_add_item, "trp_player","itm_b_samnite_sandals_cav"),
	   # (else_try),
		# (eq, ":random_footwear", 2),						
        # (troop_add_item, "trp_player","itm_samnite_single_greave_sandals"),	
	   # (else_try),
		# (eq, ":random_footwear", 3),						
        # (troop_add_item, "trp_player","itm_samnite_boots"),
	   # (else_try),
		# (eq, ":random_footwear", 4),						
        # (troop_add_item, "trp_player","itm_samnite_double_greaves_sandals"),								
	   # (try_end),	
	...
	(store_random_in_range, ":random_sword", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_sword", 3),						
        (troop_add_item, "trp_player","itm_samnite_xiphos"),
	   (else_try),
		(eq, ":random_sword", 4),						
        (troop_add_item, "trp_player","itm_w_kopis_bird_light"),								
	   (try_end),		
	...
	(store_random_in_range, ":random_spear", 0, 5),							
	   (try_begin),
		(eq, ":random_spear", 0),
        (troop_add_item, "trp_player","itm_sp_w_hasta_short"),
	   (else_try),
		(eq, ":random_spear", 1),						
        (troop_add_item, "trp_player","itm_sp_w_hasta_short"),
	   (else_try),
		(eq, ":random_spear", 2),						
        (troop_add_item, "trp_player","itm_sp_w_hasta"),	
	   (else_try),
		(eq, ":random_spear", 3),						
        (troop_add_item, "trp_player","itm_sp_w_hasta"),
	   (else_try),
		(eq, ":random_spear", 4),						
        (troop_add_item, "trp_player","itm_samnite_cavalry_lance"),								
	   (try_end),
	   
	(store_random_in_range, ":random_shield", 0, 7),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_s_aspis_samnite_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_s_aspis_samnite_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_s_aspis_samnite_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_s_aspis_samnite_4"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_s_aspis_samnite_5"),
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_sp_s_aspis_samnite_6"),
	   (else_try),
		(eq, ":random_shield", 6),						
        (troop_add_item, "trp_player","itm_sp_s_aspis_samnite_7"),	
	   (try_end),	
	...
	(store_random_in_range, ":random_horse", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_horse", 1),						
        (troop_add_item, "trp_player","itm_samnitici_equus_white"),
	   (else_try),
		(eq, ":random_horse", 2),						
        (troop_add_item, "trp_player","itm_samnitici_equus_saddle_white"),	
	   (else_try),
		(eq, ":random_horse", 3),						
        (troop_add_item, "trp_player","itm_samnitici_equus_saddle"),
	   (else_try),
		(eq, ":random_horse", 4),						
        (troop_add_item, "trp_player","itm_samnitici_equus_saddle_black"),								
	   (try_end),	 	   
	...
	# (store_random_in_range, ":random_helmet", 0, 5),							
	   # (try_begin),
		# (eq, ":random_helmet", 0),
        # (troop_add_item, "trp_player","itm_h4_samno_attic_c_red"),
	   # (else_try),
		# (eq, ":random_helmet", 1),						
        # (troop_add_item, "trp_player","itm_h_phrygian_feather"),
	   # (else_try),
		# (eq, ":random_helmet", 2),						
        # (troop_add_item, "trp_player","itm_greek_pylos_complete_7"),	
	   # (else_try),
		# (eq, ":random_helmet", 3),						
        # (troop_add_item, "trp_player","itm_h_boiotian_plume"),
	   # (else_try),
		# (eq, ":random_helmet", 4),						
        # (troop_add_item, "trp_player","itm_h6_italo_chalcidian"),								
	   # (try_end),	

	# (store_random_in_range, ":random_armor", 0, 5),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        (troop_add_item, "trp_player","itm_a_exomis_dark_blue"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_linothorax_taras_red"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_taras_square_scales_yellow"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_taras_square_scales_red"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_linothorax1_red"),								
	   # (try_end),
	...
	(store_random_in_range, ":random_shield", 0, 8),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_greek_taras_hoplon"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_plain_hoplon_4"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_39"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_greek_snake_hoplon"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_35"),
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_36"),
	   (else_try),
		(eq, ":random_shield", 6),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_37"),	
	   (else_try),
		(eq, ":random_shield", 7),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_38"),		
	   (try_end),	
	...
	# (store_random_in_range, ":random_helmet", 0, 5),							
	   # (try_begin),
		# (eq, ":random_helmet", 0),
        # (troop_add_item, "trp_player","itm_h_boeotian_v3"),
	   # (else_try),
		# (eq, ":random_helmet", 1),						
        # (troop_add_item, "trp_player","itm_h_attic_greek_white"),
	   # (else_try),
		# (eq, ":random_helmet", 2),						
        # (troop_add_item, "trp_player","itm_h6_italo_chalcidian_c_black"),	
	   # (else_try),
		# (eq, ":random_helmet", 3),						
        # (troop_add_item, "trp_player","itm_h_boiotian_plume"),
	   # (else_try),
		# (eq, ":random_helmet", 4),						
        # (troop_add_item, "trp_player","itm_thraikan_kranos_simple"),								
	   # (try_end),	

	# (store_random_in_range, ":random_armor", 0, 5),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        (troop_add_item, "trp_player","itm_a_exomis_black"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_linothorax_black"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_linothorax_syracuse"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_greek_square_scales"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_sidescale_greek_2_black"),								
	   # (try_end),
	...
	(store_random_in_range, ":random_shield", 0, 8),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_s_syracusan_hoplon_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_s_syracusan_hoplon_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_s_syracusan_hoplon_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_s_syracusan_hoplon_4"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_s_syracusan_hoplon_5"),
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_44"),
	   (else_try),
		(eq, ":random_shield", 6),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_43"),	
	   (else_try),
		(eq, ":random_shield", 7),						
        (troop_add_item, "trp_player","itm_sp_s_hoplon_40"),		
	   (try_end),	
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_helmet", 1),						
        (troop_add_item, "trp_player","itm_h_illyrian_t2_1"),
	   (else_try),
		(eq, ":random_helmet", 2),						
        (troop_add_item, "trp_player","itm_h_attic_greek_white"),	
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h_phrygian_blue_3_feather"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h_attic_mak"),								
	   (try_end),	
	...
	# (store_random_in_range, ":random_armor", 0, 5),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        (troop_add_item, "trp_player","itm_a_exomis_red"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_linothorax_red"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_linothorax_v5_epirus_red"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_linothorax_epirus_phalangite"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_linothorax_epirus_v2"),								
	   # (try_end),
	...
	(store_random_in_range, ":random_shield", 0, 8),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_4"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_5"),
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_6"),
	   (else_try),
		(eq, ":random_shield", 6),						
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_7"),	
	   (else_try),
		(eq, ":random_shield", 7),						
        (troop_add_item, "trp_player","itm_sp_s_epirus_hoplon_8"),		
	   (try_end),	
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   (try_begin),
		(eq, ":random_helmet", 0),
        (troop_add_item, "trp_player","itm_h4_italo_chalcidian_small"),
	   (else_try),
		(eq, ":random_helmet", 1),						
        (troop_add_item, "trp_player","itm_h_etrusco_thracian_t3"),
	   (else_try),
		(eq, ":random_helmet", 2),						
        (troop_add_item, "trp_player","itm_h2_negau_decorated"),	
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h2_negau_tall_crest_decorated_red"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h2_negau_sideways_crest_decorated_black"),								
	   (try_end),	

	# (store_random_in_range, ":random_armor", 0, 5),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        # (troop_add_item, "trp_player","itm_a_etruscan_cloth_white_3"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_etrusco_cardio_red_3"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        (troop_add_item, "trp_player","itm_a_etruscan_padded_linothorax"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_etrusco_lino_leather_b_white"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_etrusco_lamellar_blue_1"),								
	   # (try_end),
	...
	(store_random_in_range, ":random_sword", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_sword", 1),						
        (troop_add_item, "trp_player","itm_w_heavy_etruscan_kopis_officer"),
	   (else_try),
		(eq, ":random_sword", 2),						
        (troop_add_item, "trp_player","itm_w_light_etruscan_kopis_noble"),	
	   (else_try),
		(eq, ":random_sword", 3),						
        (troop_add_item, "trp_player","itm_w_light_etruscan_kopis_officer"),
	   (else_try),
		(eq, ":random_sword", 4),						
        (troop_add_item, "trp_player","itm_w_heavy_etruscan_kopis_noble"),								
	   (try_end),		
	...
	(store_random_in_range, ":random_shield", 0, 7),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_s_aspis_etruscan_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_s_aspis_etruscan_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_s_aspis_etruscan_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_s_aspis_etruscan_4"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_s_aspis_etruscan_5"),
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_s_aspis_etruscan_6"),
	   (else_try),
		(eq, ":random_shield", 6),						
        (troop_add_item, "trp_player","itm_s_aspis_etruscan_7"),		
	   (try_end),	
	...
	(store_random_in_range, ":random_horse", 0, 5),							
      ...
      (else_try),		
        (eq,"$background_answer_2",cb2_hoplite),
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   (try_begin),
		(eq, ":random_helmet", 0),
        (troop_add_item, "trp_player","itm_h_phrygian"),
	   (else_try),
		(eq, ":random_helmet", 1),						
        (troop_add_item, "trp_player","itm_h_phrygian_blue_1"),
	   (else_try),
		(eq, ":random_helmet", 2),						
        (troop_add_item, "trp_player","itm_h_phrygian_red"),	
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_greek_pylos"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h_boeotian_v3"),								
	   (try_end),	

	(store_random_in_range, ":random_armor", 0, 9),							
	   (try_begin),
		(eq, ":random_armor", 0),
        (troop_add_item, "trp_player","itm_a_exomis"),
	   (else_try),
		(eq, ":random_armor", 1),						
        (troop_add_item, "trp_player","itm_a_exomis_blue"),
	   (else_try),
		(eq, ":random_armor", 2),						
        (troop_add_item, "trp_player","itm_a_exomis_red"),	
	   (else_try),
		(eq, ":random_armor", 3),						
        (troop_add_item, "trp_player","itm_a_exomis_worn"),
	   (else_try),
		(eq, ":random_armor", 4),						
        (troop_add_item, "trp_player","itm_a_exomis_black"),	
	   (else_try),
		(eq, ":random_armor", 5),						
        (troop_add_item, "trp_player","itm_a_exomis_green"),	
	   (else_try),
		(eq, ":random_armor", 6),						
        (troop_add_item, "trp_player","itm_a_exomis_orange"),	
	   (else_try),
		(eq, ":random_armor", 7),						
        (troop_add_item, "trp_player","itm_a_exomis_dark_blue"),	
	   (else_try),
		(eq, ":random_armor", 8),						
        (troop_add_item, "trp_player","itm_a_exomis_yellow"),								
	   (try_end),

	(store_random_in_range, ":random_footwear", 0, 2),							
	   (try_begin),
		(eq, ":random_footwear", 0),
        (troop_add_item, "trp_player","itm_b_iphicratids"),
	   (else_try),
	...
	(store_random_in_range, ":random_sword", 0, 5),							
	   (try_begin),
		(eq, ":random_sword", 0),
        (troop_add_item, "trp_player","itm_w_machaira"),
	   (else_try),
		(eq, ":random_sword", 1),						
        (troop_add_item, "trp_player","itm_w_light_kopis_infantry"),
	   (else_try),
		(eq, ":random_sword", 2),						
        (troop_add_item, "trp_player","itm_w_kopis_infantry"),	
	   (else_try),
		(eq, ":random_sword", 3),						
        (troop_add_item, "trp_player","itm_samnite_xiphos"),
	   (else_try),
	...
	(store_random_in_range, ":random_spear", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_spear", 4),						
        (troop_add_item, "trp_player","itm_sp_w_iphicrates_doru_2"),								
	   (try_end),
	   
	(store_random_in_range, ":random_shield", 0, 8),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_greek_athenian_hoplon"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_greek_theban_hoplon"),
	   (else_try),
	   ...
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_sp_greek_rhodian_hoplon"),
	   (else_try),
		(eq, ":random_shield", 6),						
        (troop_add_item, "trp_player","itm_sp_greek_korinthian_hoplon"),	
	   (else_try),
		(eq, ":random_shield", 7),						
        (troop_add_item, "trp_player","itm_sp_plain_hoplon_4"),		
	   (try_end),		
      ...
      (else_try),
        (eq,"$background_answer_2",cb2_arjos),
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h_montefortino_boii"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h_montefortino_7"),								
	   (try_end),	
	...
	# (store_random_in_range, ":random_armor", 0, 9),							
	   # (try_begin),
		# (eq, ":random_armor", 0),
        # (troop_add_item, "trp_player","itm_a_gallic_mail_a"),
	   # (else_try),
		# (eq, ":random_armor", 1),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_b"),
	   # (else_try),
		# (eq, ":random_armor", 2),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_c"),	
	   # (else_try),
		# (eq, ":random_armor", 3),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_d"),
	   # (else_try),
		# (eq, ":random_armor", 4),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_e"),	
	   # (else_try),
		# (eq, ":random_armor", 5),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_f"),	
	   # (else_try),
		# (eq, ":random_armor", 6),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_g"),	
	   # (else_try),
		# (eq, ":random_armor", 7),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_h"),	
	   # (else_try),
		# (eq, ":random_armor", 8),						
        # (troop_add_item, "trp_player","itm_a_gallic_mail_i"),								
	   # (try_end),

        (troop_add_item, "trp_player","itm_b_celtic_pants_white"),
	...
	(store_random_in_range, ":random_spear", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_spear", 1),						
        (troop_add_item, "trp_player","itm_sp_w_arjos_spear"),
	   (else_try),
		(eq, ":random_spear", 2),						
        (troop_add_item, "trp_player","itm_sp_w_ariomaros_spear"),	
	   (else_try),
	...
	(store_random_in_range, ":random_shield", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_gallic_hexagonal_scutum_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_gallic_hexagonal_scutum_4"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_gallic_hexagonal_scutum_5"),
	   (try_end),
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   (try_begin),
		(eq, ":random_helmet", 0),
        (troop_add_item, "trp_player","itm_straw_hat"),
	   (else_try),
		(eq, ":random_helmet", 1),						
	...
	(store_random_in_range, ":random_bow", 0, 5),							
	   (try_begin),
		(eq, ":random_bow", 0),
        (troop_add_item, "trp_player","itm_cretan_bow"),
	   (else_try),
		(eq, ":random_bow", 1),						
        (troop_add_item, "trp_player","itm_greek_bow"),
	   (else_try),
		(eq, ":random_bow", 2),						
        (troop_add_item, "trp_player","itm_raw_short_bow"),	
	   (else_try),
		(eq, ":random_bow", 3),						
        (troop_add_item, "trp_player","itm_raw_hunting_bow"),
	   (else_try),
		(eq, ":random_bow", 4),						
        (troop_add_item, "trp_player","itm_greek_short_bow"),								
	   (try_end),
	   
        (troop_add_item, "trp_player","itm_arrows_2"),				
		
      (else_try),
        (eq,"$background_answer_2",cb2_phalangite),
	...
	(store_random_in_range, ":random_spear", 0, 3),							
	   (try_begin),
		(eq, ":random_spear", 0),
        (troop_add_item, "trp_player","itm_w_macedonian_sarissa_steel"),
	   (else_try),
		(eq, ":random_spear", 1),						
        (troop_add_item, "trp_player","itm_w_mak_sarissa"),
	   (else_try),
		(eq, ":random_spear", 2),						
        (troop_add_item, "trp_player","itm_w_mak_sarissa_leather"),									
	   (try_end),
	   
	(store_random_in_range, ":random_shield", 0, 8),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_4"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_5"),
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_6"),
	   (else_try),
		(eq, ":random_shield", 6),						
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_7"),	
	   (else_try),
		(eq, ":random_shield", 7),						
        (troop_add_item, "trp_player","itm_sp_mak_s_parma_8"),		
	   (try_end),		
				
      (else_try),		
        (eq,"$background_answer_2",cb2_roman_hastatus),
	...
	(store_random_in_range, ":random_helmet", 0, 5),							
	   (try_begin),
		(eq, ":random_helmet", 0),
        (troop_add_item, "trp_player","itm_h_coolus"),
	   (else_try),
		(eq, ":random_helmet", 1),						
        (troop_add_item, "trp_player","itm_h_roman_montefortino_a"),
	   (else_try),
		(eq, ":random_helmet", 2),						
        (troop_add_item, "trp_player","itm_h_roman_montefortino_a_f_red"),	
	   (else_try),
		(eq, ":random_helmet", 3),						
        (troop_add_item, "trp_player","itm_h4_samno_attic"),
	   (else_try),
		(eq, ":random_helmet", 4),						
        (troop_add_item, "trp_player","itm_h4_samno_attic_f"),								
	   (try_end),	

	(store_random_in_range, ":random_armor", 0, 10),							
	   (try_begin),
		(eq, ":random_armor", 0),
        (troop_add_item, "trp_player","itm_a_roman_tunic"),
	   (else_try),
		(eq, ":random_armor", 1),						
        (troop_add_item, "trp_player","itm_a_roman_tunic_red"),
	   (else_try),
		(eq, ":random_armor", 2),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_half"),	
	   (else_try),
		(eq, ":random_armor", 3),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_half_red"),
	   (else_try),
		(eq, ":random_armor", 4),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_small"),	
	   (else_try),
		(eq, ":random_armor", 5),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_small_red"),	
	   (else_try),
		(eq, ":random_armor", 6),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_disk_2"),	
	   (else_try),
		(eq, ":random_armor", 7),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_disk_2_red"),	
	   (else_try),
		(eq, ":random_armor", 8),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_full"),
	   (else_try),
		(eq, ":random_armor", 9),						
        (troop_add_item, "trp_player","itm_a_roman_kardio_full_red"),		
	   (try_end),

	(store_random_in_range, ":random_footwear", 0, 2),							
	   (try_begin),
		(eq, ":random_footwear", 0),
        (troop_add_item, "trp_player","itm_b_roman_caligae"),
	   (else_try),
		(eq, ":random_footwear", 1),						
        (troop_add_item, "trp_player","itm_b_roman_greaves_gutter_single"),							
	   (try_end),	
	...
	(store_random_in_range, ":random_sword", 0, 5),							
	   ...
	   (else_try),
		(eq, ":random_sword", 1),						
        (troop_add_item, "trp_player","itm_w_gladius_hispaniensis"),
	   (else_try),
		(eq, ":random_sword", 2),						
        (troop_add_item, "trp_player","itm_w_italian_xiphos_new"),	
	   (else_try),
	   ...
	   (else_try),
		(eq, ":random_sword", 4),						
        (troop_add_item, "trp_player","itm_w_espasa_2"),								
	   (try_end),		

	(store_random_in_range, ":random_spear", 0, 2),							
	   (try_begin),
		(eq, ":random_spear", 0),
        (troop_add_item, "trp_player","itm_w_light_pila"),
	   (else_try),
		(eq, ":random_spear", 1),						
        (troop_add_item, "trp_player","itm_w_heavy_pila"),								
	   (try_end),
	   
	(store_random_in_range, ":random_shield", 0, 7),							
	   (try_begin),
		(eq, ":random_shield", 0),
        (troop_add_item, "trp_player","itm_sp_hastati_scutum_1"),
	   (else_try),
		(eq, ":random_shield", 1),						
        (troop_add_item, "trp_player","itm_sp_hastati_scutum_2"),
	   (else_try),
		(eq, ":random_shield", 2),						
        (troop_add_item, "trp_player","itm_sp_hastati_scutum_3"),	
	   (else_try),
		(eq, ":random_shield", 3),						
        (troop_add_item, "trp_player","itm_sp_hastati_scutum_4"),
	   (else_try),
		(eq, ":random_shield", 4),						
        (troop_add_item, "trp_player","itm_sp_hastati_scutum_5"),
	   (else_try),
		(eq, ":random_shield", 5),						
        (troop_add_item, "trp_player","itm_sp_hastati_scutum_6"),	
	   (try_end),	
      ...
      [
	   ...
	   #plus
        # (else_try),
		  ...
		  # (set_jump_mission,"mt_lead_charge_naval_battle"),
          # (call_script, "script_setup_random_scene"),
	   #end plus		  
        (else_try),
  ...
  (
    ...
    [
		...
		(try_begin),
			...
			# (assign, reg31, "$bodysliding_last_troop"),
			# (str_store_troop_name, s31, "$bodysliding_last_troop"),
			# (display_message, "@DEBUG (EXIT): Player was last set to {s31}.", gpu_debug),
          ...
          (else_try),
            ...
            ##diplomacy end
              ...
              (else_try), #player took a walled center for player's kingdom
                ...
                (assign, ":best_troop", "trp_sp_roman_centurio"),
				##diplomacy start+
				...
				(try_begin),
				   ...
				   (neg|is_between, "$players_kingdom", npc_kingdoms_begin, npc_kingdoms_end),
				   (assign, ":best_troop", "trp_caravan_guard"),#<- while we're at it, use a mercenary default
...
# LAV MODIFICATIONS END (SIEGE CAMP ICON MINI-MOD)		  
        ...
        (try_begin),
          ...
          (store_current_hours, ":cur_hours"),
          (try_begin),
            (lt, ":cur_hours",  "$g_siege_method_finish_ours"),
            (store_sub, reg9, "$g_siege_method_finish_ours", ":cur_hours"),
            (try_begin),
              (eq, "$g_siege_method", 1),
    ...
    [
      ...
      ("castle_lead_attack",
       [
         ...
         (store_current_hours, ":cur_hours"),
         (ge, ":cur_hours", "$g_siege_method_finish_ours"),
       ],
      ...
      ("cheat_castle_lead_attack",[(eq, "$cheat_mode", 1),
       ...
       [
         ...
         (assign, "$g_siege_method_finish_ours", 0),
         (jump_to_menu, "mnu_castle_besiege"),
       ]),
    ...
    [
      ("build_ladders_cont",[],
       "Do it.", [
           ...
           (store_add, "$g_siege_method_finish_ours",":cur_hours", ":hours_takes"),
    ...
    [
	  ...
	  ##diplomacy start+ Add support for the player as co-ruler of an NPC faction
		...
		(try_begin),
			...
			(call_script, "script_cf_dplmc_print_cultural_word_to_sreg", "trp_player", DPLMC_CULTURAL_TERM_KING_FEMALE, s10),
		(else_try),
			(call_script, "script_cf_dplmc_print_cultural_word_to_sreg", "trp_player", DPLMC_CULTURAL_TERM_KING, s10),
		(try_end),
  ...
  (
    ...
    [
        ...
        (try_begin),
		  ...
		  (val_min, ":str_id", 4),
		  (try_begin),
			(is_between, "$current_town", "p_village_29", villages_end),
    ...
    [
      ...
      # ("recruit_volunteers",[(party_slot_eq, "$current_town", slot_village_state, 0),
                           # (neg|party_slot_ge, "$current_town", slot_village_infested_by_bandits, 1),
                           # ],"Recruit Volunteers.",
       # [
         # (try_begin),
         ...
         # (else_try),
           # (party_get_slot, ":village_elder_troop", "$current_town", slot_town_elder),
           # (set_mercenary_source_party, ":village_elder_troop"),
...
#           (party_add_template, "$current_town", "pt_villagers_in_raid"),
    ...
    [
      ...
      ("center_build_prisoner_tower",[(eq, reg6, 0),
       ...
       "Build a prisoner tower.",[(assign, "$g_improvement_type", slot_center_has_prisoner_tower),
                                  (jump_to_menu, "mnu_center_improve"),]),
      ("center_build_training_facility",[(eq, reg6, 0),
                                      (party_slot_eq, "$g_encountered_party", slot_party_type, spt_village),	  
                                      (party_slot_eq, "$g_encountered_party", slot_center_has_training_facility, 0),
                                       ],
       "Build a training facility.",[(assign, "$g_improvement_type", slot_center_has_training_facility),
                                  (jump_to_menu, "mnu_center_improve"),]),									  
 ...
 {reg5} denars and will take {reg6} days.",
    ...
    [
      ...
      ("improve_cont",[(store_troop_gold, ":cur_gold", "trp_player"),
       ...
       "Go on.", [(troop_remove_gold, "trp_player", reg5),
                  ...
                  (party_set_slot, "$g_encountered_party", slot_center_improvement_end_hour, ":hours_takes"),
				  (try_begin),
					  (eq, "$g_improvement_type", slot_center_has_training_facility),
					  (jump_to_menu,"mnu_center_troop_type_set"),
					(else_try),
					  (jump_to_menu,"mnu_center_manage"),
					(try_end),
                  ]),
  ...
  (
    "center_troop_type_set",0,
    "Please choose the national forces you would like to train at this location.",
    "none",
    [],
    [
      ("roman",[],"Romani.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_1"),
	...
	(try_for_range, ":village_no", villages_begin, villages_end),
		...
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
 		(party_clear, ":village_no"), # RaW		
		(try_end),
		(jump_to_menu, "mnu_center_manage"),
		]),
	  ("greek",[],"Hellenikoi.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_2"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
	  ...
	  ("gaul",[],"Celtae.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_3"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
	  ...
	  ("macedon",[],"Makedonikoi.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_4"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
	  ...
	  ("samnite",[],"Samnitici.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_5"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
	  ...
	  ("megale_hellas",[],"Tarentinoi.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_6"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
	  ...
	  ("basileion_syrakosion",[],"Syrakosioi.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_7"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
	  ...
	  ("epeiros",[],"Epeirotai.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_8"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
	  ...
	  ("razna",[],"Razna.", [(party_set_slot, "$g_encountered_party", slot_center_culture, "fac_kingdom_8"),
	(call_script,"script_update_mercenary_units_of_towns"),     
	(try_for_range, ":village_no", villages_begin, villages_end),
		(call_script, "script_refresh_village_defenders", ":village_no"),
		(call_script, "script_update_npc_volunteer_troops_in_village", ":village_no"),
...
#					(assign, "$g_last_assassination_attempt_perpetrator", ":lord"),
			...
			(try_for_range,":entry",32,41),
					...
					(try_begin),
						...
						(try_begin),
							(eq,":r",0),
							(mission_tpl_entry_add_override_item,"mt_town_default",":entry","itm_duduk"),
						(else_try),
      ...
      # ("town_arena",
       ...
       # [
           ...
           # (else_try),
             ...
             # (set_visitor, 44, "trp_caravan_guard"),
             # (set_jump_entry, 50),
 ...
 but they offer you all they can find to show their gratitude.",
  ...
  (
    ...
    [
      ...
      ("trade_with_orse_merchant",[(party_slot_ge, "$current_town", slot_town_horse_merchant, 1)],
...
##diplomacy end+
  ...
  ############### RaW begin
    (
    "raw_select_culture",0,
    "Which culture will you base your faction upon?",
    "none",
    [
    ],
    [
	...
	(assign, "$g_player_culture", "fac_kingdom_1"),	
          (change_screen_return),
        ]),
      ("greek",[],"Greek",
       [
	(faction_set_slot, "fac_player_supporters_faction",  slot_faction_culture, "fac_culture_2"),	
	(assign, "$g_player_culture", "fac_kingdom_2"),	
          (change_screen_return),
        ]),
      ("celt",[],"Celtic",
       [
	(faction_set_slot, "fac_player_supporters_faction",  slot_faction_culture, "fac_culture_3"),	
	(assign, "$g_player_culture", "fac_kingdom_3"),
          (change_screen_return),
        ]),
      ("macedonian",[],"Macedonian",
       [
	(faction_set_slot, "fac_player_supporters_faction",  slot_faction_culture, "fac_culture_4"),	
	(assign, "$g_player_culture", "fac_kingdom_4"),	
          (change_screen_return),
        ]),
      ("samnite",[],"Samnite",
       [
	(faction_set_slot, "fac_player_supporters_faction",  slot_faction_culture, "fac_culture_5"),	
	(assign, "$g_player_culture", "fac_kingdom_5"),	
          (change_screen_return),
        ]),
      ("sh_greek",[],"Southern Italy Greek",
       [
	(faction_set_slot, "fac_player_supporters_faction",  slot_faction_culture, "fac_culture_6"),	
	(assign, "$g_player_culture", "fac_kingdom_6"),	
          (change_screen_return),
        ]),
      ("syracusan",[],"Syracusan",
       [
...
##diplomacy end+
  ...
  (
    ...
    [
      ...
      ("dplmc_continue",[],"Proceed without economic advisor.",
       [
  ...
  (
    ...
    "Your economic advisor can be found at your court. You should consult him if you want to give him any financial advice or you need greater amounts of money. You should always make sure that there is enough money in the treasury to pay for political affairs.",
    "none",
    [],
    [
      ("dplmc_continue",[],"Continue...",
  ...
  (
    ...
    "As a lord of a fief you can now appoint a War Advisor who resides at you court for a weekly salary of 15 denars. He will recruit new troops and provide information about your army.",
    "none",
    [],
    [
      ...
      ("dplmc_continue",[],"Proceed without War advisor.",
       [
  ...
  (
    ...
    "Your War advisor can be found at your court. You should consult him if you want to recruit new troops or get detailed information about your standing army.",
    "none",
    [],
    [
      ("dplmc_continue",[],"Continue...",
    ...
    [
      ...
      (else_try),
		...
		(try_begin),
		   ...
		   (eq, reg0, 1),
		   (call_script, "script_cf_dplmc_print_cultural_word_to_sreg", "$g_notification_menu_var2", DPLMC_CULTURAL_TERM_KING_FEMALE, s11),
		   (assign, reg1, 1),#make sure the above didn't do anything funny with the register
		(else_try),
		   (call_script, "script_cf_dplmc_print_cultural_word_to_sreg", "$g_notification_menu_var2", DPLMC_CULTURAL_TERM_KING, s11),
