...
# #Formations Scripts	  
	...
	(try_begin),
		...
		(else_try),
			...
			(try_begin),
				...
				(call_script, "script_form_infantry", ":fteam", ":fdivision", ":fleader", ":formation_extra_spacing", ":fformation"),		# RaW		
			(try_end),
  ...
  ("form_infantry", [
	...
	(store_mul, ":extra_space", ":formation_extra_spacing", 60), # RaW was 50
	(store_add, ":distance", formation_minimum_spacing, ":extra_space"),		#minimum distance between troops	
	...
	(else_try),
		...
		(store_div, ":rank_dimension", ":num_troops", 5),		#basic three ranks # RaW -> 5 ranks now
		(val_add, ":rank_dimension", 1),		
		(assign, ":max_level", 0),
		(try_for_agents, ":agent"),
			(call_script, "script_cf_valid_formation_member", ":fteam", ":fdivision", ":fleader", ":agent"),
	...
	(else_try),
		...
		(store_div, ":rank_dimension", ":num_troops", 4),		#basic three ranks # RaW -> 4 ranks now
		(val_add, ":rank_dimension", 1),
  ...
  ("get_default_formation", [
	...
	(try_begin),
		(eq, ":ffaction", fac_kingdom_1),	#Romans
		(assign, reg0, formation_shield),
	(else_try),
		(eq, ":ffaction", fac_kingdom_2),	#Greeks
		(assign, reg0, formation_ranks),
	(else_try),
		(eq, ":ffaction", fac_kingdom_3),	#Celts
		(assign, reg0, formation_shield),	
	(else_try),
		(eq, ":ffaction", fac_kingdom_4),	#Macedonians
		(assign, reg0, formation_square),
	(else_try),
		(eq, ":ffaction", fac_kingdom_5),	#Samnites
		(assign, reg0, formation_shield),
	(else_try),
		(eq, ":ffaction", fac_kingdom_6),	#Tarentines
		(assign, reg0, formation_ranks),
	(else_try),
		(eq, ":ffaction", fac_kingdom_7),	#Syracusans
		(assign, reg0, formation_ranks),		
	(else_try),
		(eq, ":ffaction", fac_kingdom_8),	#Epeirotes
		(assign, reg0, formation_shield),
	(else_try),
		(eq, ":ffaction", fac_kingdom_9),	#Etruscans
		(assign, reg0, formation_ranks),		
	(else_try),
	    (this_or_next|eq, ":ffaction", fac_player_supporters_faction),
  ...
  ("cf_battlegroup_valid_formation", [
	...
	(else_try),
		...
		(try_begin),
			...
			# (this_or_next|eq, ":fformation", formation_shield),	# RaW		
	...
	(else_try), 
		...
		(try_for_range, ":division", 0, 9),
			...
			(try_begin),
				...
				(call_script, "script_form_infantry", "$fplayer_team_no", ":division", "$fplayer_agent_no", ":div_spacing", ":formation"),		# RaW		
			(else_try),
			    (this_or_next|team_slot_eq, "$fplayer_team_no", ":slot", sdt_cavalry),
				(team_slot_eq, "$fplayer_team_no", ":slot", sdt_harcher),
...
# RaW 
	...
	(else_try),	
		(eq, ":bgformation", formation_ranks),
		(store_mul, ":depth", ":spacing", 60),
		(val_add, ":depth", formation_minimum_spacing),
		(val_mul, ":depth", 2),		
# RaW end		
