   ...
   ("upgrade_path",0,
    ...
    [
        ...
        "{s66}",[
			...
			(store_troop_faction, ":commander_faction", "$enlisted_lord"),
			(faction_set_slot, ":commander_faction", slot_faction_enlisted_player, "$player_cur_troop"),
...
def modmerge_game_menus(orig_game_menus, check_duplicates = False):
	...
	try: ##FOR KILL_COUNT, Comment whole Try/Except out if mod includes Custom Commander
		find_i = list_find_first_match_i( orig_game_menus, "battle_debrief" )
		codeblock = GameMenuWrapper(orig_game_menus[find_i]).GetOpBlock()
		codeblock.Append( kill_count )
		#Change Text in immutable tuple by swapping to list and then back
		orig_game_menus[find_i] = list(orig_game_menus[find_i])
		orig_game_menus[find_i][2] += "^^Kill Count: {s0}" #tag to end of menu text, set in kill_count
		orig_game_menus[find_i] = tuple(orig_game_menus[find_i])
	except:
		import sys
		print "Injecton 5 failed:", sys.exc_info()[1]
		raise
